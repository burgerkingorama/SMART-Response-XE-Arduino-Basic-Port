...
const char *const errorTable[] PROGMEM = {
    ...
    string_21, string_22, string_23, string_24, string_25, string_26};
...
void printTokens(unsigned char *p) {
  ...
  while (*p != TOKEN_EOL) {
    if (*p == TOKEN_IDENT) {
      p++;
      while (*p < 0x80) host_outputChar(*p++);
      ...
      if (fmt & TKN_FMT_PRE) host_outputChar(' ');
      host_outputString((char *)pgm_read_word(&tokenTable[*p].token));
      if (fmt & TKN_FMT_POST) host_outputChar(' ');
      if (*p == TOKEN_REM) modeREM = 1;
      p++;
    }
  }
}
...
unsigned char *findProgLine(uint16_t targetLineNumber) {
  ...
  while (p < &mem[sysPROGEND]) {
    uint16_t lineNum = *(uint16_t *)(p + 2);
    if (lineNum >= targetLineNumber) break;
    p += *(uint16_t *)p;
  }
  return p;
}
...
int stackPushNum(float val) {
  if (sysSTACKEND + sizeof(float) > sysVARSTART) return 0;  // out of memory
...
int stackPushStr(char *str) {
  ...
  if (sysSTACKEND + len + 2 > sysVARSTART) return 0;  // out of memory
  unsigned char *p = &mem[sysSTACKEND];
  strcpy((char *)p, str);
  p += len;
  *(uint16_t *)p = len;
...
void stackLeftOrRightStr(int len, int mode) {
  // equivalent to popping the current string, doing the operation then pushing
  // it again
  unsigned char *p = &mem[sysSTACKEND];
...
// Array
// +--------+-------+-----------------+----------+-------+ . .
// .+-------+-------------+. . | len    | type  | name            | num dims |
// dim1  |      | dimN  | elem(1,..1) | | 2bytes | 1byte | null terminated |
// 2bytes   | 2bytes|      | 2bytes| float       |
// +--------+-------+-----------------+----------+-------+ . .
// .+-------+-------------+. .
...
unsigned char *findVariable(char *searchName, int searchMask) {
  ...
  while (p < &mem[sysVAREND]) {
    ...
    if (type & searchMask) {
      ...
      if (strcasecmp((char *)name, searchName) == 0) return p;
    }
    p += *(uint16_t *)p;
  }
  return NULL;
...
int storeNumVariable(char *name, float val) {
  ...
  if (p != NULL) {  // replace the old value
    ...
    *(float *)p = val;
    ...
    if (sysVARSTART - bytesNeeded < sysSTACKEND) return 0;  // out of memory
...
int storeForNextVariable(char *name, float start, float step, float end,
                         uint16_t lineNum, uint16_t stmtNum) {
  int nameLen = strlen(name);
  int bytesNeeded = 3;               // len + flags
  bytesNeeded += nameLen + 1;        // name
  ...
  // unlike simple numeric variables, these are reallocated if they already
  // exist since the existing value might be a simple variable or a for/next
  // variable
  unsigned char *p = findVariable(name, VAR_TYPE_NUM | VAR_TYPE_FORNEXT);
  if (p != NULL) {
...
int createArray(char *name, int isString) {
  ...
  // strings and arrays are re-allocated if they already exist
  unsigned char *p =
      findVariable(name, (isString ? VAR_TYPE_STR_ARRAY : VAR_TYPE_NUM_ARRAY));
  if (p != NULL) {
    // check there will actually be room for the new value
    uint16_t oldVarLen = *(uint16_t *)p;
    if (sysVARSTART - (bytesNeeded - oldVarLen) < sysSTACKEND)
...
int _getArrayElemOffset(unsigned char **p, int *pOffset) {
  ...
  *p += 2;
  ...
  if (numArrayDims != numDimsGiven) return ERROR_WRONG_ARRAY_DIMENSIONS;
  ...
  for (int i = 0; i < numArrayDims; i++) {
    ...
    *p += 2;
    if (index < 1 || index > arrayDim) return ERROR_ARRAY_SUBSCRIPT_OUT_RANGE;
    offset += base * (index - 1);
    base *= arrayDim;
  }
  *pOffset = offset;
...
int setNumArrayElem(char *name, float val) {
  ...
  if (p == NULL) return ERROR_VARIABLE_NOT_FOUND;
  p += 3 + strlen(name) + 1;
...
int setStrArrayElem(char *name) {
  ...
  if (sysVARSTART - bytesNeeded < oldSTACKEND) return 0;  // out of memory
  // correct the length of the variable
  *(uint16_t *)p1 += bytesNeeded;
  memmove(&mem[sysVARSTART - bytesNeeded], &mem[sysVARSTART],
          p - &mem[sysVARSTART]);
...
int gosubStackPop(int *lineNumber, int *stmtNumber) {
  if (sysGOSUBSTART == sysGOSUBEND) return 0;
  uint16_t *p = (uint16_t *)&mem[sysGOSUBSTART];
  *lineNumber = (int)*p++;
  *stmtNumber = (int)*p;
  int bytesFreed = 2 * sizeof(uint16_t);
  // shift the variable table
  memmove(&mem[sysVARSTART] + bytesFreed, &mem[sysVARSTART],
          sysVAREND - sysVARSTART);
...
int nextToken() {
  ...
  if (isalpha(*tokenIn)) {
    ...
    while (isalnum(*tokenIn) || *tokenIn == '$') {
      if (identLen < MAX_IDENT_LEN) identStr[identLen++] = *tokenIn;
      tokenIn++;
    }
    identStr[identLen] = 0;
...
// stmt number is 0 for the first statement, then increases after each command
// seperator (:) Note that IF a=1 THEN PRINT "x": print "y" is considered to be
// only 2 statements
...
int getNextToken() {
  ...
  if (curToken == TOKEN_IDENT) {
    int i = 0;
    while (*tokenBuffer < 0x80) identVal[i++] = *tokenBuffer++;
...
// parse a number
int parseNumberExpr() {
  if (executeMode && !stackPushNum(numVal)) return ERROR_OUT_OF_MEMORY;
...
int parseSubscriptExpr() {
  ...
  getNextToken();  // eat )
  if (executeMode && !stackPushNum(numDims)) return ERROR_OUT_OF_MEMORY;
  return 0;
}
...
int parseFnCallExpr() {
  ...
  for (int i = 0; i < reqdArgs; i++) {
    ...
    if (!(argTypes & 1) && !IS_TYPE_NUM(val)) return ERROR_EXPR_EXPECTED_NUM;
    if ((argTypes & 1) && !IS_TYPE_STR(val)) return ERROR_EXPR_EXPECTED_STR;
    argTypes >>= 1;
  ...
  if (executeMode) {
    ...
    switch (op) {
      ...
      case TOKEN_LEN:
        ...
        break;
      case TOKEN_SIN:
        if (!stackPushNum(sin(stackPopNum()))) return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_COS:
        if (!stackPushNum(cos(stackPopNum()))) return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_ATN:
        if (!stackPushNum(atan(stackPopNum()))) return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_ABS:
        if (!stackPushNum(abs(stackPopNum()))) return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_LN:
        if (!stackPushNum(log(stackPopNum()))) return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_EXP:
        if (!stackPushNum(exp(stackPopNum()))) return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_SQR:
        if (!stackPushNum(sqrt(stackPopNum()))) return ERROR_OUT_OF_MEMORY;
        break;
      case TOKEN_SGN:
        tmp = int(stackPopNum());
        if (tmp != 0)
      ...
      case TOKEN_VAL: {
        ...
        int val = tokenize((unsigned char *)stackGetStr(), &mem[sysSTACKEND],
                           sysVARSTART - sysSTACKEND);
        if (val) {
...
int parseIdentifierExpr() {
  ...
  if (executeMode) strcpy(ident, identVal);
...
int parseStringExpr() {
  if (executeMode && !stackPushStr(strVal)) return ERROR_OUT_OF_MEMORY;
...
// Battery is connected through a resistor divider (825k and 300k) with gain of
// 0.266666 ADC ref is 1.6V with 1024 steps to simplify the calculation we will
// neglect 1024 and round it to 1000 so we will use a conversion factor
// of 1.6/0.26666 which is 6 This will give an error of 24/1000 which is
...
int parse_INKEY() {
  getNextToken();
  if (executeMode && !stackPushNum((float)host_getKey()))
    return ERROR_OUT_OF_MEMORY;
  return TYPE_NUMBER;
}
...
int getTokPrecedence() {
  ...
  if (curToken == TOKEN_LT || curToken == TOKEN_GT || curToken == TOKEN_LT_EQ ||
      curToken == TOKEN_GT_EQ)
    return 20;
  if (curToken == TOKEN_MINUS || curToken == TOKEN_PLUS)
    return 30;
  else if (curToken == TOKEN_MULT || curToken == TOKEN_DIV ||
           curToken == TOKEN_MOD)
    return 40;
  else
    return -1;
...
int parseBinOpRHS(int ExprPrec, int lhsVal) {
  ...
  while (1) {
    ...
    if (TokPrec < ExprPrec) return lhsVal;
    ...
    if (IS_TYPE_NUM(lhsVal) && IS_TYPE_NUM(rhsVal)) {  // Number operations
      ...
      if (BinOp == TOKEN_PLUS) {
        ...
        return ERROR_UNEXPECTED_TOKEN;
    } else if (IS_TYPE_STR(lhsVal) &&
               IS_TYPE_STR(rhsVal)) {  // String operations
      if (BinOp == TOKEN_PLUS) {
        if (executeMode) stackAdd2Strs();
...
int parse_RUN() {
  ...
  if (curToken != TOKEN_EOL) {
    ...
    if (executeMode) {
      ...
      if (startLine <= 0) return ERROR_BAD_LINE_NUM;
    }
  }
  if (executeMode) {
    // clear variables
...
int parse_PAUSE() {
  ...
  if (executeMode) {
    ...
    if (ms < 0) return ERROR_BAD_PARAMETER;
    host_sleep(ms);
  }
  return 0;
}
...
int parse_LIST() {
  ...
  if (curToken != TOKEN_EOL && curToken != TOKEN_CMD_SEP) {
    int val = expectNumber();
    if (val) return val;  // error
    if (executeMode) first = (uint16_t)stackPopNum();
  }
  if (curToken == TOKEN_COMMA) {
    getNextToken();
    int val = expectNumber();
    if (val) return val;  // error
    if (executeMode) last = (uint16_t)stackPopNum();
  }
  if (executeMode) {
    listProg(first, last);
    host_showBuffer();
...
int parse_IF() {
  ...
  int val = expectNumber();
  if (val) return val;  // error
  if (curToken != TOKEN_THEN) return ERROR_MISSING_THEN;
  getNextToken();
...
int parse_FOR() {
  ...
  int val = expectNumber();
  if (val) return val;  // error
  if (executeMode) start = stackPopNum();
  // parse TO
  ...
  val = expectNumber();
  if (val) return val;  // error
  if (executeMode) end = stackPopNum();
  // parse optional STEP
  if (curToken == TOKEN_STEP) {
    getNextToken();  // eat STEP
    val = expectNumber();
    if (val) return val;  // error
    if (executeMode) step = stackPopNum();
  }
  if (executeMode) {
...
int parse_NEXT() {
  ...
  if (executeMode) {
    ...
    // loop?
    if ((data.step >= 0 && data.val <= data.end) ||
        (data.step < 0 && data.val >= data.end)) {
...
int parse_GOSUB() {
  ...
  if (executeMode) {
    ...
    if (startLine <= 0) return ERROR_BAD_LINE_NUM;
    jumpLineNumber = startLine;
    if (!gosubStackPush(lineNumber, stmtNumber)) return ERROR_OUT_OF_MEMORY;
  }
  return 0;
}
...
int parseStmts() {
  ...
  while (ret == 0) {
    ...
    if (ret || breakCurrentLine || jumpLineNumber || jumpStmtNumber) break;
    // it should either be the end of the line now, and (generally) a command
    // seperator before the next command
    if (curToken != TOKEN_EOL) {
      if (needCmdSep) {
    ...
    if (stmtNumber == targetStmtNumber) break;
  }
  return ret;
}

int processInput(unsigned char *tokenBuf) {
  // first token can be TOKEN_INTEGER for line number - stored as a float in
  // numVal store as WORD line number (max 65535)
  ...
  if (gotLineNumber) {
    ...
    while (1) {
      ...
      if (lineNumber && !jumpLineNumber && jumpStmtNumber) lineNumber = 0;
      ...
      if (!lineNumber && !jumpLineNumber && jumpStmtNumber) {
        ...
        // end of program?
        if (p == &mem[sysPROGEND]) break;  // end of program

        lineNumber = *(uint16_t *)(p + 2);
        tokenBuffer = p + 4;
        // if the target for a jump is missing (e.g. line deleted) and we're on
        // the next line reset the stmt number to 0
        if (jumpLineNumber && jumpStmtNumber && lineNumber > jumpLineNumber)
          jumpStmtNumber = 0;
      }
      if (jumpStmtNumber) targetStmtNumber = jumpStmtNumber;
